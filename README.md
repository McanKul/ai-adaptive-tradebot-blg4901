...
### 5. Web Control Panel (optional GUI)

Prefer a screen over the terminal? Every `app.py` setting is editable from a
local web UI:

```bash
python app.py web            # then open http://127.0.0.1:8000
```

Pick a subcommand (`backtest`, `walk-forward`, `live`, `dry-run`, `sweep`,
`validate`), fill in any flag, watch the live command preview, hit **Run**, and
stream the output in the browser. The form is generated by introspecting
`app.py` itself, so it always matches the CLI. You can also edit config YAMLs
and the `.env` from the same screen. See [`web/README.md`](web/README.md).

### 6. Run Live Trading (Dry-Run First!)
...
```
├── app.py              # Unified CLI entrypoint (backtest, live, dry-run, sweep, validate, web)
├── web/                # Web control panel — browser GUI over app.py (python app.py web)
