...
class TestPromotionStamp(unittest.TestCase):
    ...
    def test_creates_file_with_run_id_in_name(self):
        path = write_promotion_stamp(self.out_dir, "canary", **_common_kwargs())
    ...
    def test_contains_required_top_level_keys(self):
        ...
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        expected = {
            "run_id", "config", "strategy", "symbol", "timeframe",
            "passed_at_utc", "live_trades", "backtest_export",
            "eod_rollup", "thresholds",
        }
    ...
    def test_creates_out_dir_if_missing(self):
        ...
        path = write_promotion_stamp(nested, "canary", **_common_kwargs())
        self.assertTrue(os.path.exists(path))

    def test_run_id_propagates_into_payload(self):
        path = write_promotion_stamp(self.out_dir, "smoke_run", **_common_kwargs())
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
